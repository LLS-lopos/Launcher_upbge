...
class AutoCompleter:
    def __init__(self, text_edit):
        ...
        # Liste complète des mots-clés Python et des fonctions intégrées
        self.keywords = list(keyword.kwlist)
        self.builtin_functions = [
            'print', 'len', 'range', 'enumerate', 'zip', 'map', 'filter', 
            'str', 'int', 'float', 'list', 'dict', 'set', 'tuple', 
            'open', 'input', 'abs', 'max', 'min', 'sum', 'sorted', 'in'
        ]
        
        # Collection dynamique de mots
        self.dynamic_words = set()
    ...
    def update_completer(self):
        ...
        if filtered_words:
            ...
            self.completer.complete(cursor_rect)  # Afficher le completer à la position du curseur
